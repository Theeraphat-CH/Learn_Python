...
def Story1(win):
    def final(tl: Toplevel, name, sports, City, playername, drinkname, snacks):
        ...
        Label(tl, text=text, wraplength=tl.winfo_width().place(x=0, y=230))
...
def Story2(win):
    def final(tl: Toplevel, test, profession, noun, feeling, emotion, verb):
        text = f'''
        One day me and my friend {name} decided to play a {sports} game in {City}.
    ...
    Felling = Entry(NewScreen, width=17)
    Felling.place(x=250, y=105)

    Emotion = Entry(NewScreen, width=17)
    Emotion.place(x=250, y=150)

    Verb = Entry(NewScreen, width=17)
    Verb.place(x=250, y=250)

    SubmitButton = Button(NewScreen, text="Submit", background="Blue", font=('Times', 12), command=lambda:final(NewScreen, Noun.get(), Felling.get(), Emotion.get(), Verb.get()))
    SubmitButton.place(x=150, y=270)

#creating buttons
Story1Button = Button(Screen, text='A memorable day', font=("Times New Roman", 13),command=lambda: Story1(Screen),bg='white')
Story1Button.place(x=140, y=90)

Story2Button = Button(Screen, text='Ambitions', font=("Times New Roman", 13),command=lambda: Story2(Screen), bg='white')
